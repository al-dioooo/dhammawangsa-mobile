        ...
        <ScrollView keyboardDismissMode="interactive" contentContainerStyle={{ paddingBottom: tabBarHeight * 1.5 }} className="min-h-full bg-white dark:bg-black gap-y-4" refreshControl={<RefreshControl refreshing={isRefreshing} onRefresh={onRefresh}></RefreshControl>}>
            <View className="gap-y-4">
                ...
                <FlatList data={data} renderItem={({ item }) => (
                                ...
                                <View className="flex-row items-end justify-between">
                                    <View key={item.id} className="flex-row gap-x-4">
                                        ...
                                        <View className="justify-between">
                                            <Text>{row.name}</Text>
                                            <Text className="text-lg font-medium">{Intl.NumberFormat('id-Id', { style: 'currency', currency: "IDR" }).format(row.price)}</Text>
                                        </View>
                                    </View>
                                    <View className="flex-row items-center">
                                        <View className="p-2 rounded-full bg-neutral-200">{row.quantity === 1 ? (
                                            <IconTrash width={20} height={20} color={tailwind.colors.black} strokeWidth={1.5} />
                                        ) : (
                                            <IconMinus width={20} height={20} color={tailwind.colors.black} strokeWidth={1.5} />
                                        )}</View>
                    ...
                    <View className="w-[70%] h-[1] my-4 bg-neutral-100 self-end"></View>
                )} />
            </View>
        </ScrollView>
    )
